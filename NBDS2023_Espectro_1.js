...
(function(){
  ...
  const zoomLabel  = document.getElementById('mapZoomLevel');

  let scale = 1, tx = 0, ty = 0;
  let baseW = 0, baseH = 0; // fitted image dimensions at scale=1
  const MIN_SCALE = 1, MAX_SCALE = 8, ZOOM_STEP = 0.25;

  // Dragging state
  let isDragging = false, dragStartX = 0, dragStartY = 0, dragStartTx = 0, dragStartTy = 0;

  /** Compute the base (fitted) image size within the viewer */
  ...
  /** Clamp pan so image edges don't leave gaps (center when smaller) */
  function clampPan(){
    const vw = viewer.clientWidth;
    const vh = viewer.clientHeight;
  ...
  function resetView(){
    computeBase();
    // Start with 250% zoom and center the image
    scale = 2.5;
    tx = (viewer.clientWidth  - baseW * scale) / 2;
    ty = (viewer.clientHeight - baseH  * scale) / 2;
    // Ensure pan is clamped into valid bounds, then apply
    clampPan();
    applyTransform(true);
  }

  /** Zoom centered on viewer point (cx, cy) */
  function zoomAt(delta, cx, cy){
    const prev = scale;
    scale = Math.min(MAX_SCALE, Math.max(MIN_SCALE, scale + delta));
    if(scale === prev) return;
    const factor = scale / prev;
    // Keep point (cx,cy) fixed: newTx = cx - factor*(cx - oldTx)
    tx = cx - factor * (cx - tx);
    ty = cy - factor * (cy - ty);
    clampPan();
    applyTransform(true);
  ...
  // ── Init ──
  function init(){ resetView(); }
  img.addEventListener('load', init);
  if(img.complete && img.naturalWidth) init();
  window.addEventListener('resize', ()=>{ computeBase(); clampPan(); applyTransform(false); });
